...
public class zekra extends CircuitGenerator {
  ...
  private static int ADJLIST_SIZE = 1000;
  private static int ADJLIST_LEVELS = 15;
  private static int EXECUTION_PATH_SIZE = 1000;
      ...
      // verify forward edges (ignore empty moves to facilitate dynamic path lengths) 
      {
        ...
        if (c_h0jc0qc) {
          if (bit_h0jc0qc.getConstantValue()) {
            ...
            // retrieve the neighbour list of the current node via linear scan
            // (SmartMemory network mode truncates FieldElement values > 32 bits;
            //  linear scan uses FieldElement arithmetic which handles full BigInteger)
            FieldElement stateFieldConst = FieldElement.instantiateFrom(new BigInteger("21888242871839275222246405745257275088548364400416034343698204186575808495617"), state);
            FieldElement currNodeNeighbors = new FieldElement(new BigInteger("21888242871839275222246405745257275088548364400416034343698204186575808495617"), BigInteger.ZERO);
            for (int adjIdx = 0; adjIdx < ADJLIST_SIZE; adjIdx++) {
              Bit adjIsMatch = stateFieldConst.isEqualTo(FieldElement.instantiateFrom(new BigInteger("21888242871839275222246405745257275088548364400416034343698204186575808495617"), adjIdx));
          ...
          // (SmartMemory network mode truncates FieldElement values > 32 bits;
          //  linear scan uses FieldElement arithmetic which handles full BigInteger)
          FieldElement stateFieldNConst = FieldElement.instantiateFrom(new BigInteger("21888242871839275222246405745257275088548364400416034343698204186575808495617"), state);
          FieldElement currNodeNeighbors = new FieldElement(new BigInteger("21888242871839275222246405745257275088548364400416034343698204186575808495617"), BigInteger.ZERO);
          for (int adjIdx = 0; adjIdx < ADJLIST_SIZE; adjIdx++) {
            Bit adjIsMatch = stateFieldNConst.isEqualTo(FieldElement.instantiateFrom(new BigInteger("21888242871839275222246405745257275088548364400416034343698204186575808495617"), adjIdx));
            currNodeNeighbors = currNodeNeighbors.add(ADJLIST[adjIdx].mul(adjIsMatch));
          }
          ...
          for (int j = 0, k = 0; j < ADJLIST_LEVELS * 2; j += 2, k += (BUCKET_BITWIDTH + 8)) {
            ...
            neighborEqProd = neighborEqProd.add(neighborEq[i][j + 1].mul(FieldElement.instantiateFrom(new BigInteger("21888242871839275222246405745257275088548364400416034343698204186575808495617"), BigInteger.valueOf(2).pow(k + BUCKET_BITWIDTH))));
          }
          neighborEqProd.forceEqual(currNodeNeighbors);
